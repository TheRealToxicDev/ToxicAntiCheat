...
--[[ TAC COMMAND TO TOGGLE ANTICHEAT FEATURES ]]--
RegisterCommand("tac", function(thePlayer, args, rawCommand)
    TriggerClientEvent('ToxicAntiCheat:Toggle', -1, 1)
end)

--[[ CHECK USER FOR ADMIN OR WHITELISTED STATUS ]]--
RegisterServerEvent('ToxicAntiCheat:Whitelist')
AddEventHandler('ToxicAntiCheat:Whitelist'. function(playerId)
    ...
    if deets.group == 'admin' or deets.group == 'superadmin' or inArray(deets.identifier, Config.whitelist) then
        TriggerClientEvent('ToxicAntiCheat:WLReturn', _source, true)
    end
end)

--[[ BLACKLISTED CARS - KICK AND BAN ]]--
RegisterServerEvent('ToxicAntiCheat:Cars')
AddEventHandler('ToxicAntiCheat:Cars'. function(blacklistedCar)
    local blcar = blacklistedCar
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    print("[ToxicAntiCheat] | " ..xPlayer.name.. "["..xPlayer.identifier.. "]".._U('was_dropped_blcars'))
    TriggerClientEvent('chatMessage', -1, '^3[ToxicAntiCheat]', {255, 0, 0}, "^3" ..xPlayer.name.. "^1".._U('was_dropped_blcars'))
    ...
    TriggerEvent('ToxicAntiCheat:AutoBan', _source, banData)
end)

--[[ SUPERJUMP - KICK AND BAN ]]--
RegisterServerEvent('ToxicAntiCheat:Jump')
AddEventHandler('ToxicAntiCheat:Jump', function()
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
	print("[ToxicAntiCheat] | " ..xPlayer.name.. "[" ..xPlayer.identifier.. "] ".._U('was_dropped_superjump'))
	TriggerClientEvent('chatMessage', -1, '^3[ToxicAntiCheat]', {255, 0, 0}, "^3" ..xPlayer.name.. "^1 ".._U('was_dropped_superjump'))
...
RegisterServerEvent('ToxicAnticheat:AutoBan')
AddEventHandler('ToxicAnticheat:AutoBan', function(source, args)
	local _source = source
	local xPlayer  = ESX.GetPlayerFromId(_source)  
...
TriggerEvent('es:addGroupCommand', 'banreload', Config.permission, function (source)
	...
	if BanListLoad == true then
		...
		TriggerEvent('bansql:sendMessage', source, _U('banliist_starterror'))
	end
end)

TriggerEvent('es:addGroupCommand', 'banhistroy', Config.permission, function (source, args, user)
	if args[1] ~= nil and BanListHistory ~= {} then
		...
		if name ~= "" then
			...
			if nombre ~= nil and nombre > 0 and BanListHistory[nombre] ~= nil then
				...
				local resultat = (tostring(BanListHistory[nombre].targetplayername)) .. " , " .. (tostring(BanListHistory[nombre].sourceplayername))
		...
		if ping ~= nil and ping > 0 then
			if duree ~= nil and duree < 365 then
				...
				if duree > 0 then
					...
					local permanent = 1
					ban(source,identifier,license,liveid,xblid,discord,playerip,targetplayername,sourceplayername,duree,reason,permanent)
					DropPlayer(target, _U('you_have_been_permbanned') .. reason)
				end
