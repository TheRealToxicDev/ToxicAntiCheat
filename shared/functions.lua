TOX.TableContainsItem = function (items, table, ignoreCase)
    items = items or {}
    table = table or {}
...
TOX.PrintToUser = function(message)
    print(Ansicolors(message))
end

TOX.PrintToConsole = function(message)
    print(AnsiConsole(message))
end

TOX.RandomString = function(length)
    local result = ''
